...
Dataset format matches the original scripts (JSON stored in *.txt).
...
class App:
    ...
    def _build_menu(self) -> None:
        ...
        data_menu.add_command(label="Clear All Data", command=self.clear_dataset)
    ...
    def _build_ui(self) -> None:
        ...
        btns = ttk.Frame(left)
        btns.pack(fill=tk.X)
        ttk.Button(btns, text="Clear", command=self.clear_canvas).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 6))
        ttk.Button(btns, text="Recognize", command=self.recognize).pack(side=tk.LEFT, fill=tk.X, expand=True)

        add_box = ttk.Labelframe(left, text="Add sample", padding=10)
        add_box.pack(fill=tk.X, pady=(10, 0))
        self.label_entry = ttk.Entry(add_box)
        self.label_entry.pack(fill=tk.X)
        ttk.Button(add_box, text="Add to dataset", command=self.add_sample).pack(fill=tk.X, pady=(6, 0))

        # ----- Prediction / Training / Dataset / Log -----
        res_box = ttk.Labelframe(right, text="Prediction", padding=12)
        res_box.pack(fill=tk.X)
        self.result_var = tk.StringVar(value="—")
        self.conf_var = tk.StringVar(value="")
        ttk.Label(res_box, textvariable=self.result_var, font=("Segoe UI", 32, "bold")).pack(anchor="center")
        ttk.Label(res_box, textvariable=self.conf_var).pack(anchor="center")
        ttk.Label(res_box, text="Top 3:").pack(anchor="w", pady=(10, 0))
        self.top3 = ScrolledText(res_box, height=6, font=("Consolas", 10))
        self.top3.pack(fill=tk.X, pady=(4, 0))
        self.top3.configure(state=tk.DISABLED)

        controls = ttk.Labelframe(right, text="Training", padding=12)
        ...
        ttk.Radiobutton(model_row, text="Single", value="Single", variable=self.model_var, command=self._refresh_train_controls).pack(side=tk.LEFT)
        ttk.Radiobutton(model_row, text="MLP", value="MLP", variable=self.model_var, command=self._refresh_train_controls).pack(side=tk.LEFT, padx=(10, 0))

        self.hidden_row = ttk.Frame(controls)
        self.hidden_row.pack(fill=tk.X, pady=(10, 0))
        ttk.Label(self.hidden_row, text="Hidden", width=12).pack(side=tk.LEFT)
        self.hidden_var = tk.IntVar(value=10)
        self.hidden_scale = ttk.Scale(self.hidden_row, from_=5, to=30, variable=self.hidden_var)
        self.hidden_scale.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.hidden_lbl = ttk.Label(self.hidden_row, text="10")
        self.hidden_lbl.pack(side=tk.LEFT, padx=(8, 0))
        self.hidden_var.trace_add("write", lambda *_: self.hidden_lbl.configure(text=str(int(self.hidden_var.get()))))

        lr_row = ttk.Frame(controls)
        lr_row.pack(fill=tk.X, pady=(10, 0))
        ttk.Label(lr_row, text="Learning rate", width=12).pack(side=tk.LEFT)
        self.lr_var = tk.DoubleVar(value=0.10)
        self.lr_scale = ttk.Scale(lr_row, from_=0.01, to=1.0, variable=self.lr_var)
        self.lr_scale.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.lr_lbl = ttk.Label(lr_row, text="0.10")
        self.lr_lbl.pack(side=tk.LEFT, padx=(8, 0))
        self.lr_var.trace_add("write", lambda *_: self.lr_lbl.configure(text=f"{self.lr_var.get():.2f}"))

        ep_row = ttk.Frame(controls)
        ep_row.pack(fill=tk.X, pady=(10, 0))
        ttk.Label(ep_row, text="Epochs", width=12).pack(side=tk.LEFT)
        self.epochs_var = tk.IntVar(value=1000)
        self.epochs_scale = ttk.Scale(ep_row, from_=100, to=5000, variable=self.epochs_var)
        self.epochs_scale.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.epochs_lbl = ttk.Label(ep_row, text="1000")
        self.epochs_lbl.pack(side=tk.LEFT, padx=(8, 0))
        ...
        ttk.Button(controls, text="Train", command=self.train).pack(anchor="w")

        plot_box = ttk.Labelframe(right, text="Error Curve", padding=12)
        plot_box.pack(fill=tk.BOTH, expand=False, pady=(12, 0))
        ...
        self._plot_canvas = FigureCanvasTkAgg(self._fig, master=plot_box)
        self._plot_canvas.draw()
        self._plot_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)

        dataset_box = ttk.Labelframe(right, text="Dataset", padding=12)
        dataset_box.pack(fill=tk.BOTH, expand=True, pady=(12, 0))
        self.dataset_info = ScrolledText(dataset_box, height=12, font=("Consolas", 9))
        self.dataset_info.pack(fill=tk.BOTH, expand=True)
        self.dataset_info.configure(state=tk.DISABLED)

        log_box = ttk.Labelframe(right, text="Log", padding=12)
        log_box.pack(fill=tk.BOTH, expand=True, pady=(12, 0))
        self.log = ScrolledText(log_box, height=12, font=("Consolas", 9))
    ...
    def _paint_cell_at(self, x: int, y: int) -> None:
        ...
        if 0 <= row < GRID_ROWS and 0 <= col < GRID_COLS:
            ...
            self.canvas.itemconfigure(self._rect_ids[row][col], fill="black")
